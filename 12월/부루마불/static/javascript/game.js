...
const dice_img = ["dice1.png", "dice2.png", "dice3.png",
  "dice4.png", "dice5.png", "dice6.png"]; // 주사위 이미지 배열
...
function meeple_move() { // 주사위 값에 따라 말을 움직이기
  ...
  // 플레이어 위치 변경
  if (gamer.location + dice_sum > 31) { // 주사위값이 이동할 위치가 대전을 넘는다면
    var diff = (gamer.location + dice_sum) - 31;
    gamer.location = diff - 1;
    zone[31].func(gamer) // 출발지 통과하거나 도착하면 실행함수, 
    // zone클래스중 31번쨰가 출발지 div이다.
  } else {
  ...
  function 말위치변경(gamer, old_location) {
    // 말 위치 변경
    var old_zone = find_location(old_location); // 이동전 말 위치 찾기
    ...
    //overlap(zone_location); // 다른말과  overlap(zone_location); // 다른말과 겹치지 않게 겹치지않게
  }
}

function game_todo(location, gamer) {
  // location 매개변수는 zone클래스들중 몇번째 zone클래스인지 인덱스값 있음
  // location 값은 몇번쨰 zone클래스인지 알수도있지만, zone배열의 
  // 구역객체의 인덱스로도 사용가능
  var city = zone[location];

  if (city.purchase == 0) { // 매입금이 0인곳은 무인도,기금,출발,공항
    if (city.purchase != 0) // 출발지 도착에 대해서는 meeple_move에서 구현
      city.func(gamer);

  } else if (city.owner == '') {
    if (confirm(`${city.name}의 매입가는
    ${city.purchase}만원입니다. \n매입하실?`)) {
      if(gamer.money < city.purchase){ //토지매입가보다 플레이어자금이 적으면 구입불가
        alert("자금이 부족합니다")
        return;
      }
    ...
    var owner = city.owner; // 도시 주인
    var tollfee = city.purchase; // 통행료
    if(gamer.money < tollfee){ // 플레이어자금이 통행료보다 적다면
      player_list[owner-1].money += gamer.money;
      gamer.money = 0;
      gamer.파산 = turn; // 자금이 부족하여 파산
      파산처리(gamer); // 파산된 플레이어의 토지를 전부 매각과 토지색생제거, 밀 제거
...
function airport_move(gamer) {
  if ($(this).hasClass("center")) return;
  if (탑승객 != 0) { // 탑승객 변수에 0이 있다면 현재 이용할수 있는 플레이어가없다.

    // meeple_move에서 turn이 변경되기때문에 공항에 도착한
    // 플레이어가 위치를 선택하고 turn이 변경되어야한다.
    // 공항에 도착한 플레이어를 표현하기 위해 turn을 -1을해준다

    var 클릭한도시 = $(this);
    var 클릭한도시번호 = $(this).data("num");

    if (클릭한도시번호 == 24) { // 24는 공항num이다
